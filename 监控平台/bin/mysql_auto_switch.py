...
def mysql_switch_auto(g_dbid):
    ...
    try:
        mysqlcur=conn.cursor()
        mysqlcur.execute("set names utf8")
        t_sql="SELECT read_only,main_server,main_port,current_binlog_file,current_binlog_pos,main_binlog_file,main_binlog_pos FROM b_mysql_replication WHERE db_id=%s"
        mysqlcur.execute(t_sql,(g_dbid,))
        data = mysqlcur.fetchone()
        read_only,subordinate,port,current_binlog_file,current_binlog_pos,main_binlog_file,main_binlog_pos=data
        ...
        if read_only=='OFF':
           ...
           subordinatedb = MySQLdb.connect(host=subordinate, user=username,passwd=password,port=int(port),charset="utf8")
           subordinatecur = subordinatedb.cursor()
           ...
           if int(m_process) < 2 or int(k_process) <= 4:
              if current_binlog_file == main_binlog_file and current_binlog_pos == main_binlog_pos:
                 subordinatecur.execute(sql_off)
                 run('service keepalived stop')
                 env.user = g_h_username
                 env.host_string = subordinate+':22'
                 env.password = g_h_passwd
...
def mysql_switch_manual(g_dbid):
    ...
    env.password = g_h_passwd
    if read_only=='OFF':
       maindb = MySQLdb.connect(host=ip_addr, user=username,passwd=password,port=int(port),charset="utf8")
       ...
       print 'DBID: '+g_dbid+' is subordinate! Please chose a main!'
